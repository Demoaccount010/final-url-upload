...
from database import add_file_record, remove_file_record
import os
from database import consume_quota
...
async def youtube_dl_call_back(_bot, update):
    ...
    # --- QUOTA CHECK ADDED (button) ---
    try:
        allowed = await consume_quota(update.from_user.id)
    except Exception:
        allowed = True
    if not allowed:
        try:
            await update.message.edit_caption(
                caption=(
                    ...
                    "Aapka free daily limit khatam ho gaya. /buy"
                )
            )
        except Exception:
            await _bot.send_message(chat_id=update.from_user.id, text="⚠️ Daily limit reached. /buy")
        return False
    # --- END QUOTA CHECK (button) ---
    custom_file_name = (
    ...
    try:
        await add_file_record(update.from_user.id, os.path.basename(download_directory))
    except Exception:
        pass
