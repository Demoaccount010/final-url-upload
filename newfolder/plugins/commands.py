...
async def give_premium(bot, m):
    try:
        ...
        return await m.reply_text("Owner not configured in Config. OWNER_ID missing.")

    if m.from_user.id != owner_id:
        return await m.reply_text("Only owner can run this command.")
...
async def redeem_code(bot, m):
    ...
    expiry = res
    await m.reply_text(f"🎉 Promo accepted! Premium active till {expiry.strftime('%d-%m-%Y')}")
...
async def broadcast_cmd(bot, m):
    ...
    await m.reply_text(f"✅ Broadcast finished. Sent: {sent} Failed: {failed}")
...
async def set_limit_cmd(bot, m):
    owner = int(Config.OWNER_ID)
    if m.from_user.id != owner:
        return await m.reply_text("❌ Only owner can set custom limit.")
    parts = m.text.split()
    if len(parts) != 3:
...
async def profile_cmd(bot, m):
    ...
    await m.reply_text(
        ...
        f"🔗 Used Today: <b>{usage['limit_used']}</b> / {usage['daily_limit']}\n"
...
async def ref_cmd(bot, m):
    ...
    await m.reply_text("🎉 Thanks! Your referral has been recorded.")
